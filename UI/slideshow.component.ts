        ...
        @if (!store.loading() && store.hasPhotos()) {
          <div class="stage"
            ...
            @if (store.activePhoto(); as photo) {
              <div class="photo-frame" [class.transitioning]="transitioning()">
                @if (photo.signedViewUrl) {
                  <img
                    class="photo-img"
                    [src]="photo.signedViewUrl"
                    [alt]="photo.caption ?? 'Photo ' + (store.activeIndex() + 1)"
                    (load)="onImageLoad()"
                    (error)="onImageError()"
                  />
                } @else {
            ...
            <button class="nav-btn nav-prev"
                    [class.hidden]="store.isFirstPhoto()"
                    (click)="goTo('prev')"
                    aria-label="Previous photo">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 18 9 12 15 6"/></svg>
            </button>
            <button class="nav-btn nav-next"
                    [class.hidden]="store.isLastPhoto()"
                    (click)="goTo('next')"
          ...
          <div class="dots" role="tablist" aria-label="Photo navigation">
            @for (photo of store.photos(); track photo.id; let i = $index) {
              <button
                ...
                role="tab"
                [attr.aria-selected]="i === store.activeIndex()"
              ></button>
            }
          </div>
        }
